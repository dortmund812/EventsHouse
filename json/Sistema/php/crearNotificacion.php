...
if (isset($_POST['titulo'])
	...
	if ($usuario == 1) {
		...
		foreach ($secretarias as $secretaria) {
			$secret = $secretaria[0];
			crearNotificacion($conexion, $titulo, $mensaje, $userRec[0], $secret);
		}
		$salida .= 'Exito';
	} else if ($usuario == 2) {
		...
		foreach ($jefeBodega as $jefe) {
			$user = $jefe[0];
			crearNotificacion($conexion, $titulo, $mensaje, $userRec[0], $user);
		}
		$salida .= 'Exito';
	} else if ($usuario == 3) {
		...
		crearNotificacion($conexion, $titulo, $mensaje, $userRec[0], $seleccionado[0]);
		$salida .= 'Exito';
	} else {
		$salida .= 'Error';
	}
